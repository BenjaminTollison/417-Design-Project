...
inlet_pressure = inlet_stagnation_pressure * (
    inlet_temperature / inlet_stagnation_temperature
) ** (
...
radius_tip_inlet = (
    mass_flow_rate
    / (np.pi * inlet_density * velocity_axial * (1 - inlet_hub_tip_ratio**2))
) ** 0.5  # m
...
critical_inlet_area = (
    inlet_area * (AreaRatioFromMach(inlet_mach_number, gamma_air)) ** -1
)
outlet_temperature_stagnation = inlet_stagnation_temperature * (
    compressor_pressure_ratio
) ** ((1 / 0.84544) * ((gamma_air - 1) / gamma_air))
outlet_temperature = compressor_outlet_total_temperature - velocity_axial**2 / (
    2 * 1005
)  # K
outlet_pressure_stagnation = compressor_pressure_ratio * inlet_stagnation_pressure  # Pa
outlet_pressure = outlet_pressure_stagnation * (
    outlet_temperature / outlet_temperature_stagnation
) ** (
    gamma_air / (gamma_air - 1)
)  # Pa
...
annulus_dimensions = {
    ...
    "inlet_radii": [radius_hub_inlet, radius_mean_intlet, radius_tip_inlet],
    "outlet_radii": [radius_hub_outlet, radius_mean_outlet, radius_tip_outlet],
}
annulus_dimensions = {
    key: [
        float(item) if isinstance(item, (np.float64, np.float32)) else item
        for item in value
    ]
